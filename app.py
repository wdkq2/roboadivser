...
TRADE_API_URL = os.getenv("TRADE_API_URL", "https://example.com/trade")
...
def execute_trade(symbol, amount):
    """Send an order to the trading API."""
    try:
        amt = float(amount)
    except ValueError:
        return "Invalid amount"
    payload = {"api_key": TRADE_API_KEY, "symbol": symbol, "amount": amt}
    try:
        resp = requests.post(TRADE_API_URL, json=payload, timeout=10)
        resp.raise_for_status()
        portfolio[symbol] = portfolio.get(symbol, 0) + amt
        data = resp.json()
        msg = data.get("message", "trade executed")
        return f"{msg} 현재 보유 {portfolio[symbol]}원"
    except Exception as e:
